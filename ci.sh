...
if [[ -z ${CI_OPT_CI_SCRIPT+x} ]]; then CI_OPT_CI_SCRIPT="${CI_INFRA_OPT_GIT_PREFIX}/ci-and-cd/maven-build/raw/v1.0.3/src/main/ci-script/lib_ci.sh"; fi
...
if [[ -z ${CI_OPT_MAVEN_BUILD_REPO+x} ]]; then CI_OPT_MAVEN_BUILD_REPO="${CI_INFRA_OPT_GIT_PREFIX}/ci-and-cd/maven-build/raw/v1.0.3"; fi
if [[ -z ${CI_OPT_MAVEN_EFFECTIVE_POM+x} ]]; then CI_OPT_MAVEN_EFFECTIVE_POM="false"; fi
if [[ -z ${CI_OPT_ORIGIN_REPO_SLUG+x} ]]; then if [[ -n "${CI_PROJECT_PATH}" ]]; then CI_OPT_ORIGIN_REPO_SLUG="home1-oss/log-config"; else CI_OPT_ORIGIN_REPO_SLUG="home1-oss/log-config"; fi; fi
...
if [[ -z "${CI_OPT_SITE_PATH_PREFIX}" ]] && [[ "${CI_OPT_GITHUB_SITE_PUBLISH}" == "true" ]]; then
    ...
    CI_OPT_SITE_PATH_PREFIX="cloud-ready/log-config"
fi
